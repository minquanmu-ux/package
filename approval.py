"""
企业微信审批管理CLI工具
启动欢迎/检查 userid，支持汉字状态、自动补全日期、操作提示
（不包含同意/拒绝审批功能）
"""
import requests
import time
            ...
            if no_status:
                ...
                for i, sp in enumerate(sp_list, 1):
                    try:
                        ...
                        status_cn = "获取失败"
                    click.echo(f"  {i:3d}. {sp}  [{status_cn}]")
                    time.sleep(0.1)

        click.echo("\n💡 下一步操作：")
        click.echo("  查看详情：  python approval.py detail -n <单号>")
        click.echo("  待处理审批：python approval.py process")
    ...
    try:
        if sp_no:
            ...
            if info.get("sp_status") == 1:
                click.echo("\n💡 该单据审批中，请在手机端或电脑端企业微信完成审批。")
            else:
                click.echo("\n💡 该单据已处理，无需操作。")
                click.echo("   查询其他单据：python approval.py list -s <开始日期> -e <结束日期>")
        else:
            ...
            if not sp_list:
                ...
                for idx, sp in enumerate(sp_list, 1):
                    ...
                    time.sleep(0.1)
                click.echo("\n💡 下一步操作：")
                click.echo("  查看待处理审批：python approval.py process")
    except Exception as e:
        click.echo(f"❌ 错误：{e}", err=True)
...
def process(start, end, days, sp_no):
    ...
    try:
        ...
        if sp_no:
            ...
            if not sp_list:
                click.echo("🎉 没有待审批的单据！")
                click.echo("💡 提示：可扩大范围重试，例如 python approval.py process --days 14")
                return
            pending = []
            for sp in sp_list:
                try:
                    ...
                    pending.append((sp, info.get("sp_name", ""), info.get("applyer", {}).get("userid", ""),
                                    info.get("apply_time", 0)))
                except:
                    pass
            click.echo(f"待审批单据 {len(pending)} 个：")
            for i, (sp, name, applyer, ts) in enumerate(pending, 1):
                if ts > 1e12:
                    ts //= 1000
                t = datetime.fromtimestamp(ts).strftime("%m-%d %H:%M") if ts else ""
                click.echo(f"  [{i}] {sp} | {name} | {applyer} | {t}")
            click.echo("\n💡 请在手机端或电脑端企业微信完成审批。")
    except Exception as e:
        click.echo(f"❌ 错误：{e}", err=True)
